...
public class ChromecastHandler {
	...
	public static VideoCastManager getVideoCastManager(Context ctx) { 
		...
		//mCastMgr.setContext(ctx); TODO this was removed
		return mCastMgr;
	}
	...
	public static void initialize( Context context, OnCastDeviceChangeListener deviceChangeListener, OnCastRouteDetectedListener routeDetectedListener ) {
    	...
    	mDialog.setContentView(com.vice.kaltura_player.R.layout.media_routers);
    	mDialog.setTitle( Html.fromHtml("<font color='#0db6d1'>Connect to Device</font>"));
    	Button disconnect = (Button) mDialog.findViewById(com.vice.kaltura_player.R.id.disconnect);
        if ( disconnect != null) {
		...
		final ListView listview = (ListView) mDialog.findViewById(com.vice.kaltura_player.R.id.listview);
